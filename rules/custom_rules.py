...
class CustomS3PublicAccessBlockRule(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for name, resource in cfn.get_resources(['AWS::S3::BucketPublicAccessBlock']).items():
            ...
            for key in ('BlockPublicAcls','IgnorePublicAcls','BlockPublicPolicy','RestrictPublicBuckets'):
                if key in pab and pab[key] is False:
                    path = ['Resources', name, 'Properties', 'PublicAccessBlockConfiguration', key]
    ...
    def match(self, cfn):
        ...
        for name, res in cfn.get_resources(['AWS::S3::Bucket']).items():
            ...
            if not found_good:
                ...
                matches.append(RuleMatch(path, f"{name} has no AES256/aws:kms server-side encryption"))
        return matches
...
class CustomIamWildcardActionRule(CloudFormationLintRule):
        ...
        for i, st in enumerate(stmts):
            ...
            for a in act_list:
                if a == '*' or a.endswith(':*') or a == '*:*':
                    ...
                    matches.append(RuleMatch(path, f"{resname} policy Action uses wildcard: {a}"))
    ...
    def match(self, cfn):
        ...
        for name, res in cfn.get_resources(['AWS::IAM::Policy']).items():
            ...
            self._check_policy_doc(pd, ['Resources', name, 'Properties', 'PolicyDocument'], matches, name)
        # Roles / inline policies
        for name, res in cfn.get_resources(['AWS::IAM::Role','AWS::IAM::User','AWS::IAM::Group']).items():
            props = res.get('Properties', {}) or {}
            policies = props.get('Policies') or []
            for idx, pol in enumerate(policies):
                pd = pol.get('PolicyDocument') or {}
                self._check_policy_doc(pd, ['Resources', name, 'Properties', 'Policies', idx, 'PolicyDocument'], matches, name)
        return matches
...
class CustomSecurityGroupOpenToWorldRule(CloudFormationLintRule):
    ...
    def _port_contains_danger(self, from_p, to_p):
        try:
            ...
            if to_p is None:
                return True
            return (from_p <= 22 <= to_p) or (from_p <= 3389 <= to_p) or (from_p == 0 and to_p == 65535)
        except Exception:
            return False
                ...
                for idx, rule in enumerate(rules):
                    ...
                    if cidr == '0.0.0.0/0' or cidr == '::/0':
                        if self._port_contains_danger(from_p, to_p):
                            ...
                            matches.append(RuleMatch(path, f"{name} {prop_name}[{idx}] open to the world on dangerous ports"))
        return matches
        ...
        for t in ('AWS::RDS::DBInstance', 'AWS::RDS::DBCluster'):
            for name, res in cfn.get_resources([t]).items():
                ...
                if se is not True:
                    ...
                    matches.append(RuleMatch(path, f"{name} missing StorageEncrypted=true"))
        return matches
...
class CustomCloudFrontHttpsOnlyRule(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for name, res in cfn.get_resources(['AWS::CloudFront::Distribution']).items():
            ...
            default_cache_behavior = distribution_config.get('DefaultCacheBehavior') or {}
            viewer_policy = default_cache_behavior.get('ViewerProtocolPolicy')

            if viewer_policy != 'redirect-to-https':
                path = ['Resources', name, 'Properties', 'DistributionConfig', 'DefaultCacheBehavior', 'ViewerProtocolPolicy']
...
class CustomVpcFlowLogsEnabledRule(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for name, resource in cfn.get_resources(['AWS::EC2::VPC']).items():
            ...
            found_log = any(
                'ResourceId' in fl.get('Properties', {}) and fl['Properties']['ResourceId'] == {'Ref': name}
                for fl in flow_logs
            )
            if not found_log:
                path = ['Resources', name]
                matches.append(RuleMatch(path, f"{name}: VPC Flow Logs are not enabled for this VPC"))
        return matches
    ...
    def match(self, cfn):
        ...
        for name, resource in cfn.get_resources(['AWS::Lambda::Function']).items():
            ...
            for var_name, var_value in variables.items():
                if any(x in var_name.upper() for x in ('PASSWORD', 'SECRET', 'API_KEY', 'TOKEN')):
                    path = ['Resources', name, 'Properties', 'Environment', 'Variables', var_name]
    ...
    def _check_policy(self, policy_doc, base_path, matches, resname):
        ...
        for i, statement in enumerate(statements):
            ...
            if principals == '*' or (isinstance(principals, dict) and principals.get('AWS') == '*'):
                ...
                matches.append(RuleMatch(path, f"{resname}: SNS Topic policy is overly permissive with a wildcard principal"))
...
class CustomWafEnabledOnCloudFront(CloudFormationLintRule):
    ...
    def match(self, cfn):
        ...
        for name, res in cfn.get_resources(['AWS::CloudFront::Distribution']).items():
            ...
            if not web_acl_id:
                path = ['Resources', name, 'Properties', 'DistributionConfig', 'WebACLId']
    ...
    def match(self, cfn):
        ...
        for name, res in cfn.get_resources(['AWS::S3::Bucket']).items():
            ...
            if access_control in ('PublicRead', 'PublicReadWrite'):
                ...
                matches.append(RuleMatch(path, f"{name}: S3 bucket ACL '{access_control}' is too permissive"))
        return matches
